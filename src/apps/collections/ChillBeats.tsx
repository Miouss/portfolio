  ...
  return (
    <ChillBeatsContainer>
      ...
      <PlayerButtons>
        ...
        <Play onClick={(e: React.MouseEvent<HTMLButtonElement>) => playAudio(e, audioFile, setIsPlaying)}>
          {isPlaying ? <PauseIcon /> : <PlayIcon />}
        </Play>
        <SkipForward onClick={() => skipForward(track, setTrack, trackList)}>
          <SkipForwardIcon />
        </SkipForward>
      </PlayerButtons>
      <SoundControl>
        ...
        <VolumeSlider
          ...
          max={100}
          step="1"
          onChange={(e: ChangeEvent<HTMLInputElement>) => changeVolume(e, setVolume)}
        />
      </SoundControl>
    </ChillBeatsContainer>
  );
